...
const columns = [
  ...
  },
  {
    ...
    width: 120,
    customRender: ({ record }: { record: ModelDeployment }) => {
      const statusMap: Record<DeploymentStatus, { color: string; icon: any; label: string }> = {
...
const filteredList = computed(() => {
  return deploymentList.value.filter((item) => {
    ...
    if (searchParams.model_source && item.model_source !== searchParams.model_source) {
      return false;
    }
    return true;
  });
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'name'">
          <a @click="handleDetail(record)" class="text-blue-500 hover:text-blue-600">
        ...
        <template v-if="column.key === 'endpoint'">
          ...
          <span v-else class="text-gray-400">{{ record.endpoint || '未分配' }}</span>
        </template>
        ...
        <template v-if="column.key === 'action'">
          <Space>
            ...
            <template v-if="record.status === 'running'">
              <Tooltip title="停止">
                <Button type="text" size="small" @click="handleStop(record.id)">
                  <PauseCircleOutlined class="text-orange-500" />
                </Button>
              </Tooltip>
              <Tooltip title="重启">
                <Button type="text" size="small" @click="handleRestart(record.id)">
                  <ReloadOutlined class="text-purple-500" />
                </Button>
              </Tooltip>
            </template>
            <template v-else-if="record.status === 'stopped' || record.status === 'failed'">
              <Tooltip title="启动">
                <Button type="text" size="small" @click="handleStart(record.id)">
        ...
        <Tabs>
          <TabPane key="info" tab="基本信息">
            <Descriptions bordered :column="3" size="small">
              <DescriptionsItem label="部署名称">{{ record.name }}</DescriptionsItem>
              <DescriptionsItem label="模型名称">{{ record.model_name }}</DescriptionsItem>
              <DescriptionsItem label="模型版本">{{ record.model_version }}</DescriptionsItem>
              <DescriptionsItem label="容器名称">{{ record.container_name }}</DescriptionsItem>
              <DescriptionsItem label="镜像名称">{{ record.image_name }}</DescriptionsItem>
              <DescriptionsItem label="端口">{{ record.port }}</DescriptionsItem>
              <DescriptionsItem label="副本数">{{ record.replicas }}</DescriptionsItem>
              <DescriptionsItem label="部署节点">{{ record.node_name }}</DescriptionsItem>
              <DescriptionsItem label="创建者">{{ record.created_by }}</DescriptionsItem>
            </Descriptions>
          </TabPane>
          ...
          <TabPane key="nginx" tab="Nginx配置">
            <Descriptions bordered :column="2" size="small">
              ...
              <DescriptionsItem label="SSL">
                <Badge
                  :status="record.nginx_config?.ssl_enabled ? 'success' : 'default'"
                  :text="record.nginx_config?.ssl_enabled ? '已启用' : '未启用'"
                />
              </DescriptionsItem>
            </Descriptions>
          </TabPane>
