...
const filteredBuckets = computed(() => {
  ...
  return result.filter((bucket) => {
    ...
    if (bucket.type === 'public') {
      ...
      return (
        Array.isArray(authorizedTenants) &&
        authorizedTenants.includes(tenantUid)
      );
    }
    return false;
  });
...
// 镜像读取失败保留明确状态，不显示空仓库。 Preserve image read failures instead of presenting an empty repository.
const loadDockerImages = async () => {
  try {
    loadingImages.value = true;
    imageReadError.value = '';
    const response = await getDockerFiles();
    dockerImages.value = response || [];
  } catch (error) {
    dockerImages.value = [];
    imageReadError.value = publicRequestMessage(error, '镜像列表读取失败');
...
const handleOperatorChange = (operatorId: number) => {
  ...
  if (operator) {
    ...
    for (const [key, param] of Object.entries(
      selectedOperator.value.parameterSchema,
    )) {
...
const handleDatabaseChange = async (database: string) => {
  ...
  try {
    ...
    if (
      currentVersion === tableRequestVersion.value &&
      formState.database === currentDatabase
    ) {
      tables.value = result;
    }
  } catch (error) {
...
const handleTableChange = async (tableName: string) => {
  ...
  try {
    ...
    if (
      currentVersion === columnRequestVersion.value &&
      formState.tableName === currentTableName
    ) {
      ...
      if (selectedOperator.value && !fieldFeatureConfigs.value[columnName]) {
        ...
        for (const col of selectedOperator.value.featureColumns) {
          if (col.default !== undefined) {
            ...
            fieldFeatureConfigs.value[columnName][col.key] =
...
// 更新字段特征配置
const updateFieldFeatureConfig = (
  columnName: string,
  key: string,
  value: any,
) => {
...
const handleDatasetSelect = (value: string) => {
  ...
  // 查找是否是已有数据集
  const existingDataset = filteredDatasets.value.find(
    (ds) => ds.dataset_file === value,
  );
  if (existingDataset) {
...
const validateStep2 = (): boolean => {
  ...
  if (scheduleConfigState.isActive) {
    ...
    switch (scheduleConfigState.intervalType) {
      ...
      case 'weekly':
        if (
          scheduleConfigState.weeklyDays.length === 0 ||
          !scheduleConfigState.weeklyTime
        ) {
      ...
      case 'interval':
        if (
          !scheduleConfigState.intervalDuration ||
          !scheduleConfigState.intervalUnit ||
          !scheduleConfigState.dateRange ||
          scheduleConfigState.dateRange.length < 2
        ) {
      ...
      case 'monthly':
        if (
          !scheduleConfigState.monthlyDay ||
          !scheduleConfigState.monthlyTime
        ) {
...
const handleSave = async () => {
  ...
  try {
    // 构建调度配置JSON
    const scheduleConfigJson = scheduleConfigState.isActive
      ? JSON.stringify({
          ...
          dateRange: scheduleConfigState.dateRange?.map(
            (d: any) => d?.toISOString?.() || d,
          ),
          ...
          onceTime:
            scheduleConfigState.onceTime?.toISOString?.() ||
            scheduleConfigState.onceTime,
    ...
    // 构建通知配置JSON
    const notificationConfigJson = notificationConfigState.isActive
      ? JSON.stringify({
    ...
    const data: FeatureEngineering = {
      ...
      teamUid: isEditMode.value
        ? formState.teamUid
        : selectedOrg.value.teamUid || '',
...
const loadEditData = async (id: number) => {
  ...
  try {
    ...
    // 设置已选算子
    if (data.operatorId) {
      const operator = featureOperators.value.find(
        (op) => op.id === data.operatorId,
      );
      if (operator) {
        selectedOperator.value = parseFeatureOperator(operator);
      }
    ...
    if (data.notificationConfig) {
      try {
        ...
        notificationConfigState.notificationTitle =
          notificationData.notificationTitle || '';
        notificationConfigState.notificationContent =
          notificationData.notificationContent || '';
        notificationConfigState.notificationUserID =
          notificationData.notificationUserID || '';
        notificationConfigState.notificationTrigger =
          notificationData.notificationTrigger || 'on_failure';
      } catch (e) {
...
const columnTableColumns = computed(() => {
  ...
  if (
    ...
    selectedOperator.value.featureColumns.length > 0
  ) {
    for (const col of selectedOperator.value.featureColumns) {
      baseColumns.push({
        title: col.title,
...
<template>
  <BusinessPage
    domain="数据准备"
    description="从数据集、特征到知识库，组织好训练与检索所需的数据。"
  >
          ...
          <Row :gutter="24">
            ...
            <Col :span="10">
              <Card title="基本信息" class="mb-4">
                <Form.Item label="特征工程名称" name="name" :rules="nameRules">
              ...
              <Card title="镜像配置" class="mb-4">
                ...
                <Alert
                  ...
                  type="error"
                  show-icon
                  :message="imageReadError"
                  class="image-read-error mb-3"
                >
                  <template #action
                    ><Button :loading="loadingImages" @click="loadDockerImages"
                      >重试读取</Button
                    ></template
                  >
                </Alert>
              ...
              <Card title="特征算子配置" class="mb-4">
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Select.Option
                        v-for="op in featureOperators.filter(
                ...
                <div v-if="selectedOperator" class="operator-info mb-4">
                  ...
                  <div class="flex items-center gap-2">
                    <Tag color="purple">{{
                      getCategoryLabel(selectedOperator.category)
                    }}</Tag>
                    <Tag
                ...
                <Collapse
                  v-if="
                    selectedOperator &&
                    Object.keys(selectedOperator.parameterSchema).length > 0
                  "
                >
              ...
              <Card title="输出配置">
                ...
                <template v-if="formState.pushToDataset">
                  <Form.Item
                    label="目标数据集"
                    :rules="[
                      { required: true, message: '请输入或选择数据集名称' },
                    ]"
                  >
                    <AutoComplete
                    ...
                    <div class="mt-1 text-xs text-gray-400">
                      <template v-if="formState.targetDatasetId">
                        <Tag color="green" size="small">已有数据集</Tag>
                        将追加到此数据集
                      </template>
                      <template v-else-if="formState.targetDatasetName">
                        <Tag color="orange" size="small"
                          ><PlusOutlined /> 新建数据集</Tag
                        >
                        将创建新数据集
                      </template>
                      <template v-else>
                  ...
                  <template #bodyCell="{ column, record }">
                    <template v-if="column.key === 'select'">
                      <Checkbox
                        ...
                        @change="
                          (e: any) =>
                            handleColumnSelect(record.name, e.target.checked)
                        "
                      />
                    </template>
                    ...
                    <template v-else-if="column.key?.startsWith('feature_')">
                      <template
                        v-for="featureCol in selectedOperator?.featureColumns"
                        :key="featureCol.key"
                      >
                        <template
                          v-if="column.key === `feature_${featureCol.key}`"
                        >
                          <template
                            v-if="selectedColumns.includes(record.name)"
                          >
                            <!-- Select 类型 -->
                            <Select
                              v-if="featureCol.type === 'select'"
                              :value="
                                fieldFeatureConfigs[record.name]?.[
                                  featureCol.key
                                ]
                              "
                              @change="
                                (v: any) =>
                                  updateFieldFeatureConfig(
                                    record.name,
                                    featureCol.key,
                                    v,
                                  )
        ...
        <Row :gutter="24">
          <Col :span="24">
            ...
            <Card class="mb-4">
              ...
              <div
                v-if="scheduleConfigState.isActive"
              ...
              >
                ...
                <!-- 每天调度配置 -->
                <Row
                  v-if="scheduleConfigState.intervalType === 'daily'"
                  :gutter="16"
                >
                  <Col :span="8">
                ...
                <!-- 每周调度配置 -->
                <Row
                  v-if="scheduleConfigState.intervalType === 'weekly'"
                  :gutter="16"
                >
                  <Col :span="12">
                ...
                <!-- 每月调度配置 -->
                <Row
                  v-if="scheduleConfigState.intervalType === 'monthly'"
                  :gutter="16"
                >
                  <Col :span="8">
                ...
                <!-- 每小时调度配置 -->
                <Row
                  v-if="scheduleConfigState.intervalType === 'hourly'"
                  :gutter="16"
                >
                  <Col :span="8">
                ...
                <!-- Cron表达式 -->
                <Row
                  v-if="scheduleConfigState.intervalType === 'cron'"
                  :gutter="16"
                >
                  <Col :span="12">
                ...
                <!-- 预约调度 -->
                <Row
                  v-if="scheduleConfigState.intervalType === 'once'"
                  :gutter="16"
                >
                  <Col :span="8">
                    <Form.Item label="执行时间">
            ...
            <Card>
              ...
              <div
                v-if="notificationConfigState.isActive"
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'usage'">
                <div>
                  ...
                  <div
                    class="mt-1 h-2 rounded-full bg-gray-200 dark:bg-gray-700"
                  >
                    <div
      ...
      >
        <div class="image-selector">
          ...
          <Table
            ...
            :custom-row="
              (record: DockerFile) => ({
                onClick: () => handleImageRowClick(record),
              })
            "
          >
            <template #bodyCell="{ column, record }">
